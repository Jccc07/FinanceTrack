      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: H + 20, paddingBottom: 20 }}>
        {data.map(d => (
          <div key={d.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--text3)', whiteSpace: 'nowrap' }}>{d.month}</span>
          </div>
        ))}
      </div>
    </div>
...
function DonutChart({ segments }: { segments: { label: string; value: number; color: string }[] }) {
  ...
  if (total === 0) return <p style={{ textAlign: 'center', color: 'var(--text3)', fontSize: 13, padding: '20px 0' }}>No expense data</p>
          ...
          return (
            <div key={seg.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--text2)' }}>{cat?.label ?? seg.label}</span>
              </div>
              <span style={{ fontSize: 12, color: 'var(--text3)', fontWeight: 600 }}>{((seg.value / total) * 100).toFixed(1)}%</span>
            </div>
          )
        })}
      </div>
...
export function AnalyticsPage() {
  ...
  return (
    <div className="fade-in">
      <h1 style={{ fontFamily: 'var(--font-head)', fontSize: 24, fontWeight: 800, marginBottom: 20 }}>Analytics</h1>
      ...
      {isLoading
        ...
        : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: 'var(--bg2)', borderRadius: 'var(--radius-lg)', padding: '16px 14px', border: '1px solid var(--border)' }}>
              <h2 style={{ fontFamily: 'var(--font-head)', fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Income vs Expenses — 6 months</h2>
              <BarChart data={data ?? []} />
            </div>

            {/* This month summary */}
            <div style={{ background: 'var(--bg2)', borderRadius: 'var(--radius-lg)', padding: '16px', border: '1px solid var(--border)' }}>
              <h2 style={{ fontFamily: 'var(--font-head)', fontWeight: 700, fontSize: 15, marginBottom: 14 }}>This Month</h2>
              {thisMonth && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                  {[
                    ...
                    <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
                      ...
                      <span style={{ fontSize: 15, fontWeight: 700, color: row.color }}>₱{Math.abs(row.value).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Category breakdown */}
            <div style={{ background: 'var(--bg2)', borderRadius: 'var(--radius-lg)', padding: '16px', border: '1px solid var(--border)' }}>
              <h2 style={{ fontFamily: 'var(--font-head)', fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Spending by Category</h2>
              <DonutChart segments={topCats} />
            </div>

            {/* Monthly table */}
            <div style={{ background: 'var(--bg2)', borderRadius: 'var(--radius-lg)', padding: '16px', border: '1px solid var(--border)' }}>
              <h2 style={{ fontFamily: 'var(--font-head)', fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Monthly Summary</h2>
              <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 300 }}>
                  <thead>
                    <tr>
                      {['Month', 'Income', 'Expenses', 'Net'].map(h => (
                        <th key={h} style={{ textAlign: h === 'Month' ? 'left' : 'right', padding: '8px 10px', color: 'var(--text3)', fontWeight: 600, borderBottom: '1px solid var(--border)', fontSize: 12 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[...(data ?? [])].reverse().map(row => (
                      <tr key={row.month}>
                        <td style={{ padding: '10px', color: 'var(--text)', fontWeight: 500 }}>{row.month}</td>
                        <td style={{ padding: '10px', textAlign: 'right', color: 'var(--green)', fontWeight: 600 }}>₱{row.income.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                        <td style={{ padding: '10px', textAlign: 'right', color: 'var(--red)', fontWeight: 600 }}>₱{row.expense.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                        <td style={{ padding: '10px', textAlign: 'right', fontWeight: 700, color: row.net >= 0 ? 'var(--green)' : 'var(--red)' }}>
